...
#ifdef _KARIN_WEBVIEW
#include <QtGui/QFileDialog>
#include <QtGui/QDesktopServices>
#include <QNetworkRequest>

#define _DECLARATIVE_WEBVIEW_INIT \
    bDontDelegateLinks(true)
#endif
...
bool QDeclarativeWebPage::javaScriptConfirm(QWebFrame* originatingFrame, const QString& msg)
{
    ...
    // by yeatse: to launch confirm dialog
...
bool QDeclarativeWebPage::javaScriptPrompt(QWebFrame* originatingFrame, const QString& msg, const QString& defaultValue, QString* result)
{
    ...
    // by yeatse: to implement this function
...
void QDeclarativeWebView::ClearHistory()
{
}

bool QDeclarativeWebView::CanGoForward() const
{
    return history()->canGoForward();
}

bool QDeclarativeWebView::CanGoBack() const
{
    return history()->canGoBack();
}

int QDeclarativeWebView::HistoryCount() const
{
    return history()->count();
}

bool QDeclarativeWebView::DontDelegateLinks() const
{
    return bDontDelegateLinks;
}

void QDeclarativeWebView::SetDontDelegateLinks(bool b)
{
    if(bDontDelegateLinks != b)
    {
        bDontDelegateLinks = b;
        SetLinkDelegationPolicy();
        emit dontDelegateLinksChanged(bDontDelegateLinks);
    }
}

void QDeclarativeWebView::SetLinkDelegationPolicy(QWebPage *p)
{
    QWebPage *cp;

    cp = p ? p : page();
    if(cp)
        cp->setLinkDelegationPolicy(bDontDelegateLinks ? QWebPage::DontDelegateLinks : QWebPage::DelegateAllLinks);
}

void QDeclarativeWebView::SetupPage(QWebPage* p)
{
    QWebPage *page;
...
void QDeclarativeWebPage::javaScriptConsoleMessage ( const QString & message, int lineNumber, const QString & sourceID )
{
    ...
    emit viewItem()->javaScriptConsoleMessage(message, lineNumber, sourceID);
}
